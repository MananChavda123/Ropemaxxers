...
public:
    ...
    bool isSwMemoryOperation,memoryData;
...
int memoryAccess;
    int misses;
    Cache2(int size, int bSize, int assoc, int latency) 
        : cacheSize(size), blockSize(bSize), associativity(assoc), accessLatency(latency) {
        numSets = cacheSize / (blockSize * associativity);
        memoryAccess = 0;
        misses = 0;
...
public:
    ...
    processor(int cacheSize, int blockSize, int associativity, int cacheLatency, int memoryLatency): cache(cacheSize, blockSize, associativity, cacheLatency),cache2(cacheSize*4, blockSize, associativity, cacheLatency) {
        memory = new int[1024];
...
void independentRun() {
    ...
    while (!core0_finished || !core1_finished) {
        if (!core0_finished) {
            ...
            isSwMemoryOperation = cores[0].isSwMemoryOperation;
            if(isMemoryOperation){
                if(!cache.accessCache(cores[0].address,false,data)){
                    cache.accessLatency = cache.accessLatency+10;
                    if(!cache2.accessCache(cores[0].address,false,data)){
                            cache.accessLatency = cache.accessLatency+40;
                    }
                }
            }
            else if(isSwMemoryOperation){
                data.push_back(cores[0].memoryData);
                if(!cache.accessCache(cores[0].address,true,data)){
                    cache.accessLatency = cache.accessLatency+10;
                    if(!cache2.accessCache(cores[0].address,false,data)){
                            cache.accessLatency = cache.accessLatency+40;
                    }
        ...
        if (!core1_finished) {
            ...
            isSwMemoryOperation = cores[1].isSwMemoryOperation;
            if(isMemoryOperation){
                if(!cache.accessCache(cores[1].address,false,data)){
                    cache.accessLatency = cache.accessLatency+10;
                    if(!cache2.accessCache(cores[1].address,false,data)){
                            cache.accessLatency = cache.accessLatency+40;
                    }
                }
            }
            else if(isSwMemoryOperation){
                data.push_back(cores[1].memoryData);
                if(!cache.accessCache(cores[1].address,true,data)){
                    cache.accessLatency = cache.accessLatency+10;
                    if(!cache2.accessCache(cores[1].address,false,data)){
                            cache.accessLatency = cache.accessLatency+40;
                    }
                ...
                cores[1].isSwMemoryOperation = false;
...
int main() {
    processor p(1024, 64, 4, 10, 50);//Cache size is initially taken 8kB,vary cache size and associativity as per your need.
